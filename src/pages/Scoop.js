...
import { fetchScoopDetails, upvoteScoop } from '../redux/actions/scoopActions';
...
const Scoop = () => {
  ...
  const scoopDetails = useSelector((state) => state.scoopDetails);

  useEffect(() => {
    // Fetch scoop details when component mounts
    dispatch(fetchScoopDetails(scoopId));
  }, [dispatch, scoopId]);

  const handleUpvote = async (scoopId) => {
    console.log('scoopId:', scoopId); // Log the scoopId
    try {
      await dispatch(upvoteScoop(scoopId));
      ...
      <div className=' flex space-x-32 max-md:flex-col max-md:space-x-0 max-md:space-y-10'>
        <section className='mt-9 grow'>
          ...
          <h1 className='text-[43px] leading-[48px] max-sm:text-3xl text-text max-w-4xl font-extrabold mb-4 relative'>{news?.headline}
            <button onClick={() => handleUpvote(scoopId)}
              className='font-FF text-xs p-0 py-0.5 tracking-widest font-bold px-5 bg-green-600 absolute bottom-2 ml-5'>UPVOTE</button>
          </h1>
